...
import {
  ...
  ListChecks, CheckCircle2, Sparkles, Shield, UserCheck, DollarSign,
} from "lucide-react";
...
/* ════════════════════════════════════════════════════════════════════════════
   VARIANT 1 — "Watch, Then Convert" (native mobile app pattern)
   Lands directly in the settled layout: a centered headline on top, video
   left / form right (same height, no scroll) below it on desktop, a short
   description, and a single left-to-right process row anchors the bottom.
   On mobile, the form, process steps, and quick facts live in bottom sheets
   opened from a fixed bottom tab bar, and a translucent top app bar carries
   the wordmark — nothing else on the page to scroll to, so there's no
   scroll-spy tab. The reel loops in the background.
   ════════════════════════════════════════════════════════════════════════════ */
...
/** Same reasons-to-choose-us as VideoPageV2's "Get All Three" section —
 *  V1 has no inline section to show these in, but the Decide sheet should
 *  still match V2's content instead of just repeating the hero's stats. */
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full">
        ...
        <div className="px-6 sm:px-10 lg:px-16 max-w-container mx-auto w-full pt-16 lg:pt-6 sm:pt-8 text-center">
          <motion.img
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 lg:items-center gap-6 lg:gap-10 px-6 sm:px-10 lg:px-16 max-w-container mx-auto w-full">
          ...
          >
            <div className="w-full rounded-2xl border border-gray-200 bg-white shadow-2xl p-4 sm:p-5">
              ...
              <LeadForm idPrefix="video-v1-desktop" />
            </div>
  ...
  return (
    ...
    >
      ...
      <MobileInfoSheet open={formOpen} onClose={() => setFormOpen(false)} title="Let&apos;s Start Today">
        <LeadForm idPrefix="video-v1-mobile" onSubmitted={() => setFormOpen(false)} />
      </MobileInfoSheet>

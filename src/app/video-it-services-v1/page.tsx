...
import {
  ...
  ListChecks, CheckCircle2, Sparkles, UserCheck, Film, DollarSign,
} from "lucide-react";
...
/* ════════════════════════════════════════════════════════════════════════════
   VARIANT 1 — "Watch, Then Convert" ("mobile app" iteration)
   Lands directly in the settled layout: a centered headline on top, video
   left / form right (same height) below it, a short description, and a
   single left-to-right process row anchors the bottom on desktop. On mobile:
...
/** Same reasons-to-choose-us as VideoITServicesPageV2's "Get All Three"
 *  section — V1 has no inline section to show these in, but the Decide
 *  sheet should still match V2's content instead of just repeating the
 *  hero's stats. */
...
/** Kept as its own component (rather than a plain string constant) so the
 *  original copy's line break renders identically everywhere it's used. */
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 lg:items-center gap-6 lg:gap-10 px-6 sm:px-10 lg:px-16 max-w-container mx-auto w-full">
          ...
          >
            <div className="w-full rounded-2xl border border-gray-200 bg-white shadow-2xl p-4 sm:p-5">
              ...
              <LeadForm idPrefix="video-it-v1-desktop" />
            </div>
  ...
  return (
    ...
    >
      ...
      <MobileInfoSheet open={formOpen} onClose={() => setFormOpen(false)} title="Let&apos;s Start Today">
        <LeadForm idPrefix="video-it-v1-mobile" onSubmitted={() => setFormOpen(false)} />
      </MobileInfoSheet>
